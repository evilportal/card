...
function show_banner() {
    ...
    if [ -f "banner.txt" ]; then
        ...
        echo -e "${redColour}[!]${endColour}${grayColour} No se encontró el archivo banner.txt${endColour}"
    fi
...
function startAttack(){
	...
	echo -ne "${yellowColour}[*]${endColour}${grayColour} Plantilla a utilizar (card):${endColour} " && read template
